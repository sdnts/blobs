...
export const withIp: Middleware = (request, env): void | Response => {
...
export const withRayId: Middleware = (request): void | Response => {
...
export const withTunnel: Middleware = (request, env) => {
  const { tunnelId } = request;
  if (!tunnelId) {
    console.error({ tunnelId }, "Invalid tunnelId");
    return error(400, "Bad pathname");
  }
